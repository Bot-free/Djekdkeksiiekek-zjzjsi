...
def send_likes(uid):
    try:
        likes_api_response = requests.get(
            f"http://foubia-like-ff.vercel.app/like?key=FoUbIaaa&uid=%7Buid%7D&region=ME"
        )
...
def  generate_random_word():
    word_list = [
        ...
        "يافاشل", "نيكمك", "كوسمك", "يكسمك", "هضرب_عشره_عليك", "لحاااااس", " VEX_chef", "TELEGRAM:@tmgx_kira"
    ]
...
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][FFB300]Credits:MA7ROUS AND YOUNES
        """
        return time
...
def spam_requests(player_id):
    url = f"http://m3-adhbhom-spam.vercel.app/request?uid={player_id}"
    res = requests.get(url)
    if res.status_code() == 200:
        print("Done-Spam")
    else:
...
def Increase_visits(player_id):
    ...
    [C][B][11EAFD]‎━━━━━━━━━━━━
    [C][B][FFB300]Credits: [FFFFFF]MA7ROUS AND  [00FF00]YOUNES !
    """
        return msg_visit
    else:
...
‎━━━━━━━━━━━━
[808080][B][C]Credits: MA7ROUS AND YOUNES
    """
        return msg_visit
    else:
...
[FFFFFF]لدعم مشروعك والحصول على سيرفر قوي ومستقر، تواصل مباشرة مع المطور محروس عبر:

[00FF00]تيليجرام: @YMCMA7ROUS
[00FF00]إنستجرام: YMCMA7ROUS 

[FFFFFF]رأيت الأوامر؟ أرسل:
[00FF00]/🗿help

[C][B][00FFFF]━━━━━━━━━━━━━━━
[C][B][808080]Credits: MA7ROUS AND YOUNES 
""",uid
                            )
                        )
                else:
            ...
            if '1200' in data.hex()[0:4] and b'/lag' in data:
                try:
                    ...
                    parsed_data = json.loads(json_result)
                    uid = parsed_data['5']['data']['1']['data']
                    cmd = parsed_data['5']['data']['4']['data']
                    room_id = cmd.split()[1]

        # إرسال رسالة بدء)
                    clients.send(
                    self.GenResponsMsg(f"[C][B][FF6347]تم  [C][B][32CD32]الوضع [C][B][000000]في  [C][B][FF4500]مقبري بنجاح", uid))

        # تنفيذ 60 دورة من الدخول والخروج
                    for _ in range(800):
                    	join_teamcode(socket_client, room_id, key, iv)
                    	time.sleep(0.001)  # سرعة الدخول
                    	socket_client.send(self.leave_s())
                    	time.sleep(0.001)  # سرعة الخروج
                except Exception as e:
                	print(f"حدث خطأ أثناء تنفيذ codex: {e}")
...
إقبل طلب بسرعة!!!
━━━━━━━━━━━━
[{generate_random_color()}]Credits: MA7ROUS AND YOUNES 
            """, uid))
            ...
            if "1200" in data.hex()[0:4] and b"/m/" in data:
                try:
                    ...
                    try:
                        ...
                        id_match = re.search(r'/m/(\d{5,15})\b', cleaned_message)
                        ...
                        if id_match:
                            ...
                            temp_id = cleaned_message.split('/m/')[1].split()[0].strip()
...
[808080][B][C]Credits:MA7ROUS AND YOUNES 
            ...
            if '1200' in data.hex()[0:4] and b'/start' in data:
                try:
                    ...
                    # إرسال رسالة مع ألوان متعددة
          #          clients.send(gen_msgv2_clan(f"[C][B][A9D16D] تم بدء الاختراق ", key, iv))
         #           time.sleep(1.50)  
                    clients.send(
                    self.GenResponsMsg(f"[C][B][FF6347]??? [C][B][32CD32]Squad Hacked [C][B][000000]Starting... (⌐■-■) [C][B][FF4500]Warning: Effect lasts for all", uid))
                    
                    # إجراء العمليات التالية مع التوقيت
                    time.sleep(0.50)  
                    join_teamcode(socket_client, room_id, key, iv)
                    time.sleep(5)  
                    started(socket_client, key, iv)  
                    time.sleep(0.50)  
                    started(socket_client, key, iv)  
                    time.sleep(0.50)  
                    started(socket_client, key, iv)  
                    ...
                    # في حالة حدوث خطأ يتخطى الخطأ ويكمل السكربت
                    print(f"حدث خطأ: {e}")
                    pass     
            ...
            if "1200" in data.hex()[0:4] and b"/spam/" in data:
                try:
                    ...
                    try:
                        import re
                        id_match = re.search(r'/spam/(\d{5,15})\b', cleaned_message)
                        ...
                        if id_match:
                            ...
                            parts = cleaned_message.split('/spam/')
                            if len(parts) > 1:
                                temp_id = parts[1].split()[0].strip()
                                if temp_id.isdigit() and 5 <= len(temp_id) <= 15:
                                    player_id = temp_id
                    		...
                    		if player_id.isdigit():
                                ...
                                self.GenResponsMsg(
                                    f"\n{generate_random_color()}- SpAm StArtEd For uid {fix_num(player_id)} !\n", uid
                            			...
                            			clients.send(
                                self.GenResponsMsg(
                                    f"\n\n\n{generate_random_color()} [00FF00]يا مصري  !\n\n\n", uid
                                )
                            )
                        		else:
                        		      clients.send(
...
[FF0000]‎━━━━━━━━━━━━
            ...
            if "1200" in data.hex()[0:4] and b"/MAC" in data:
                ...
                clients.send(
                    self.GenResponsMsg(
                        f"""[b][c][FF000F] المايك [FF298E]غير مبند""", uid
                    )
                )
                
            if "1200" in data.hex()[0:4] and b"/FM" in data:
                i = re.split("/FM", str(data))[1]
                if "***" in i:
                    i = i.replace("***", "106")
                sid = str(i).split("(\\x")[0]
                json_result = get_available_room(data.hex()[10:])
                ...
                clients.send(
                    self.GenResponsMsg(
                        f"""[b][c][FF000F]أدخل للقائمة المهتم بها واجعلها ضمن قائمتك
...
[C][B] [FF0000]--------------------------------""", uid
            ...
            if "1200" in data.hex()[0:4] and b"/HED" in data:
                ...
                clients.send(
                    self.GenResponsMsg(
                        f"""[b][c][FF000F]   جدات عليك يعطيك عصب ههههههههههه""", uid
                    )
                )
            if "1200" in data.hex()[0:4] and b"/XTZ" in data:
                i = re.split("/XTZ", str(data))[1]
                if "***" in i:
                    i = i.replace("***", "106")
                sid = str(i).split("(\\x")[0]
                json_result = get_available_room(data.hex()[10:])
                ...
                uid = parsed_data["5"]["data"]["1"]["data"]
                clients.send(
                    self.GenResponsMsg(f"""jadet 3lik y3tek 3'sba hh""", uid))
            ...
            if "1200" in data.hex()[0:4] and b"/DEV" in data:
                ...
                clients.send(
                    self.GenResponsMsg(
                        f"""[b][c][FF000F]♥
[FF0000َ]مرحبا بكم في بوت MA7ROUS  OFFICIAL
[C][B] [C][FF0000]--------------------------------
[FFFF00] أسم المطور الحقيقي هو تيم قواقزه
[C][B] [FF0000]--------------------------------
[00FFFFَ] لشراء سورس بوت صديق تواصل معي انستا او تليجرام
[C][B] [FF0000]--------------------------------
[C][B] [FF0000]سيتم إضافة ميزات جديده قريبا
[C][B] [FE8EF9] المطور MA7ROUS OFFICIAL للتواصل معي تليجرام   @YMCMA7ROUS""", uid
                    )
                )
            ...
            if "1200" in data.hex()[0:4] and b"/help" in data:
	                ...
	                clients.send(
		                    self.GenResponsMsg(
		                        f"""[cُ][bَ][FFB300]WELCOME TO MA7ROUS BOT ENJOY :)
	
	[{generate_random_color()}] معلومات الاعب  vip
	
	[{generate_random_color()}] / info/  <id>
	
	[C][B] [FF0000]--------------------------------
	
	[C][B] [FFD700] زيادة لايكات   vip
	
	[C][B] [FF6347] / likes/  <id>

	[C][B] [FF0000]--------------------------------
	
	[C][B] [FFD700] تحقق من بند  vip الحسابات
	
	[C][B] [FF6347] / check/ <id>
	
	[C][B] [FF0000]--------------------------------
	
	[C][B] [FFD700]  تكلم مع ذكاء الأصطناعي
	
	[C][B] [FF6347] / ai <The word>
	
	[C][B] [FF0000]--------------------------------
	
	[C][B] [FFD700] سبام طلبات صدقة 
	
	[C][B] [FF6347] / spam/ <id> 
	
	[C][B] [FF0000]--------------------------------	
				
	[C][B] [FFD700] زيادة زوار في الايدي  vip

	[C][B] [FF6347] / vist/ <id>""", uid
		                    )
		                )
            ...
            if "1200" in data.hex()[0:4] and b"/help" in data:
	                ...
	                clients.send(
		                    self.GenResponsMsg(
		                        f"""[C][B][ffff00]حالة الكود VIP  لشراء كود توصل معي 
	[C][B] [FF0000]--------------------------------
	
	[C][B] [00f0f0] خمسة في السكواد
	
	[C][B] [B290FF] / 3s
	
	[C][B] [FF0000]--------------------------------
	
	[C][B] [00f0f0] تحسن الانترنت
	
	[C][B] [B290FF] / om chef

	[C][B] [FF0000]--------------------------------
	
	[C][B] [00f0f0] لأضافة قائمة امنيات
	
	[C][B] [B290FF] / FM
	
	[C][B] [FF0000]--------------------------------
	
	[C][B] [00f0f0] سبام طلبات روم
	
	[C][B] [B290FF] / room/  <id>
	
	[C][B] [FF0000]--------------------------------
	
	[C][B] [00f0f0] سبام دعوة للفريق
	
	[C][B] [B290FF] / inv <id> 5 or 6 or 3 or 3

	[C][B] [FF0000]--------------------------------	
				
	[C][B] [00f0f0] تحقق من بند المايك في الايدي
	...
	[FFD700]/DEV""", uid
            ...
            if "1200" in data.hex()[0:4] and b"/help" in data:
	                ...
	                clients.send(
		                    self.GenResponsMsg(
		                        f"""[C][B] [FF397E] فتح سكواد خمسة لصديقك
	 [C][B][00FF00]/ 5s [id]

	 [C][B] [0000ff]--------------------------------
			
	 [C][B] [FF398E]فتح سكواد ستة لصديقك
     [C][B][00FF00]/ 6s [id]
    
     [C][B] [FF0000]--------------------------------
     
     [C][B] [FFFFFF]اعرف اذا شخص شغال بلايدي
     [C][B][00FF00] / s <id>
		 
     [C][B] [FF00FF]--------------------------------

     [C][B] [FFFFFF]اعرف مين بسكواد صاحبك 
     
     [C][B] [00FF00]/ status <id>
     
     [C][B] [E84FFE]--------------------------------
     
     [C][B] [00FF00]نصائح للهيدشوت
     ...
     [C][B] [00FF00]/ HED""", uid
            ...
            if "1200" in data.hex()[0:4] and b"/help" in data:
	                ...
	                clients.send(
		                    self.GenResponsMsg(
		                        f"""[cُ][bَ][FFB300]Hey [C][B][FF0100] نرحب بك في المقبرة
                         لشراء توصل معي 

[C][B] [EF98E0]حالة الكود VIP

[C][B] [EF98E0]--------------------------------

[C][B][FF0100] المميزات و الاوامر

[C][B] [EF98E0]--------------------------------

[C][B][FF0100] إضافة لاعب للمقبرة 
[C][B][FF02FE] / XTZ id

[C][B] [EF98E0]--------------------------------

[C][B] [FF0000] إزالة الاعب من المقبرة
[C][B] [EF98E0]@ZZ id

[C][B] [EF98E0]--------------------------------

[C][B] [FF0000]حالة المقبروين
[C][B] [EF98E0]/ZI id

[C][B] [EF98E0]--------------------------------

[C][B] [FF0000]عرض جميع المقبورين
[C][B] [EF98E0]@ZII id
...
[C][B] [EF98E0]--------------------------------""", uid
    ...
    def guest_token(self,uid , password):
        ...
        headers = {"Host": "100067.connect.garena.com","User-Agent": "GarenaMSDK/4.0.19P4(G011A ;Android 11;en;EN;)","Content-Type": "application/x-www-form-urlencoded","Accept-Encoding": "gzip, deflate, br","Connection": "close",}
...
if __name__ == "__main__":
    try:
        client_thread = FF_CLIENT(id="444", password="password")
        client_thread.start()
    except Exception as e:
